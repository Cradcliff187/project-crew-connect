...
const Dashboard = () => {
  ...
  const { data: counts = { projects: 0, workOrders: 0, vendors: 0, invoices: 0 }, isLoading } = useQuery({
    ...
    queryFn: async () => {
      try {
        const [projectsResponse, workOrdersResponse, vendorsResponse] = await Promise.all([
          ...
          supabase.from('maintenance_work_orders').select('work_order_id', { count: 'exact', head: true }),
