...
export const useEstimateSubmit = () => {
  ...
  const submitEstimate = async (
    ...
    try {
      ...
      // Insert estimate with estimateid as null to let the database generate it
      // The database trigger will use the generate_estimate_id() function
      const { data: newEstimate, error } = await supabase
        .from('estimates')
        .insert({
          // estimateid will be generated by the database trigger
          ...
          estimateid: null // Explicitly set to null to trigger the database function
        })
        .select('estimateid')
        .single();
