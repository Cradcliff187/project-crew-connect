...
const usePdfGeneration = ({ onSuccess, onError }: UsePdfGenerationProps = {}) => {
  ...
  const generateClientSidePdf = useCallback(
    async (contentRef: React.RefObject<HTMLDivElement>) => {
      setIsGenerating(true);
      setError(null);
      ...
      try {
        ...
        const projectName =
          content.querySelector('[data-field="project-name"]')?.textContent || 'N/A';
        const estimateNumber =
          content.querySelector('[data-field="estimate-number"]')?.textContent || 'N/A';
        const customerName =
          content.querySelector('[data-field="customer-name"]')?.textContent || 'N/A';
        const date =
          content.querySelector('[data-field="date"]')?.textContent ||
