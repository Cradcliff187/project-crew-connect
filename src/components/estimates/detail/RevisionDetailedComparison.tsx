...
const RevisionDetailedComparison: React.FC<RevisionDetailedComparisonProps> = ({
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const typeSafeCurrentItems: EstimateItem[] = (currentItemsData || []).map(item => ({
        ...item,
        item_type: (item.item_type || 'other') as
          | 'labor'
          | 'material'
          | 'subcontractor'
          | 'vendor'
  ...
  const processComparisonData = (current: EstimateItem[], compare: EstimateItem[]) => {
    ...
    const changed = current
      .map(currentItem => {
        const previousItem = compare.find(
          ci =>
        ...
        if (
          ...
          const percentageDifference =
            previousItem.total_price !== 0 ? (priceDifference / previousItem.total_price) * 100 : 0;
  ...
  const totalChangedItemsDifference = changedItems.reduce(
  ...
  const percentageChange =
    compareRevision?.amount && compareRevision.amount > 0
      ...
      <CardContent>
        ...
        <div className="mb-6 grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-slate-50 p-4 rounded-md border">
            ...
            <div
              className={`text-xl font-medium ${netDifference >= 0 ? 'text-green-600' : 'text-red-600'}`}
            >
          ...
          <div className="bg-blue-50 p-4 rounded-md border border-blue-100">
            ...
            <div
              className={`text-xl font-medium ${totalChangedItemsDifference >= 0 ? 'text-green-600' : 'text-red-600'}`}
            >
                  ...
                  <TableBody>
                      ...
                      <>
                        ...
                        <TableRow key={`${item.current.id}-total`}>
                          ...
                          <TableCell className="text-right font-medium">
                            <span
                              className={
                                item.priceDifference >= 0 ? 'text-green-600' : 'text-red-600'
                              }
                            >
                    ...
                    <TableRow className="bg-blue-50">
                      ...
                      <TableCell className="text-right font-medium">
                        <span
                          className={
                            totalChangedItemsDifference >= 0 ? 'text-green-600' : 'text-red-600'
                          }
                        >
