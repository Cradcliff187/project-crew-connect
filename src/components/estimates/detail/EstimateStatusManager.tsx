...
const EstimateStatusManager: React.FC<EstimateStatusManagerProps> = ({
  ...
  function getAvailableStatusOptions(status: string): StatusOption[] {
    ...
    switch (lowerStatus) {
      case 'draft':
        return standardStatusOptions.filter(opt => ['sent'].includes(opt.value.toLowerCase()));
      case 'sent':
      case 'pending':
      ...
      case 'approved':
        return standardStatusOptions.filter(opt => ['converted'].includes(opt.value.toLowerCase()));
      case 'rejected':
        return standardStatusOptions.filter(opt => ['draft'].includes(opt.value.toLowerCase()));
      default:
        return [];
    }
  }
