...
import { PieChart, Pie, Cell, ResponsiveContainer, Legend, Tooltip as RechartsTooltip, 
...
const RevisionFinancialComparison: React.FC<RevisionFinancialComparisonProps> = ({
  ...
  const fetchRevisionData = async () => {
    try {
      ...
      calculateMetrics(currentRevData, compareRevData, currentItemsData || [], compareItemsData || []);
    } catch (error) {
  ...
  const calculateMetrics = (
    ...
    const categorySummary = categories.map(category => {
      const currentCategoryItems = currentItems.filter(item => (item.item_type || 'Other') === category);
      const compareCategoryItems = compareItems.filter(item => (item.item_type || 'Other') === category);
      
      const currentCategoryTotal = currentCategoryItems.reduce((sum, item) => sum + item.total_price, 0);
      const compareCategoryTotal = compareCategoryItems.reduce((sum, item) => sum + item.total_price, 0);
      ...
      return {
        ...
        percentChange: compareCategoryTotal > 0 
          ? (categoryDifference / compareCategoryTotal) * 100 
          : (currentCategoryTotal > 0 ? 100 : 0)
      };
    ...
    const pieChartData = {
      current: categories.map(category => {
        const categoryItems = currentItems.filter(item => (item.item_type || 'Other') === category);
        return {
          name: category,
      ...
      compare: categories.map(category => {
        const categoryItems = compareItems.filter(item => (item.item_type || 'Other') === category);
        return {
          name: category,
          value: categoryItems.reduce((sum, item) => sum + item.total_price, 0)
        };
    ...
    return (
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card className={revisionMetrics.totalDifference >= 0 ? "border-green-200" : "border-red-200"}>
            <CardContent className="pt-6">
              <div className="flex flex-col">
                <div className="text-sm text-muted-foreground">Total Change</div>
                <div className={`text-2xl font-semibold mt-2 ${revisionMetrics.totalDifference >= 0 ? "text-green-600" : "text-red-600"}`}>
                ...
                <div className="flex items-center mt-1">
                  ...
                  <span className={`text-sm ${revisionMetrics.percentChange >= 0 ? "text-green-600" : "text-red-600"}`}>
                    {revisionMetrics.percentChange > 0 ? '+' : ''}
          ...
          <Card className={revisionMetrics.marginComparison.difference >= 0 ? "border-green-200" : "border-red-200"}>
            <CardContent className="pt-6">
              <div className="flex flex-col">
                <div className="text-sm text-muted-foreground">Margin Change</div>
                <div className={`text-2xl font-semibold mt-2 ${revisionMetrics.marginComparison.difference >= 0 ? "text-green-600" : "text-red-600"}`}>
                ...
                <div className="flex items-center mt-1">
                  ...
                  <span className="text-sm">
                    ...
                    <span className={`font-medium ${revisionMetrics.marginComparison.current.percentage >= revisionMetrics.marginComparison.previous.percentage ? "text-green-600" : "text-red-600"}`}>
                      {' '}{revisionMetrics.marginComparison.current.percentage.toFixed(1)}%
                    </span>
                  </span>
                </div>
                <div className="mt-3 pt-3 border-t text-sm flex justify-between">
                  <span>Previous: {formatCurrency(revisionMetrics.marginComparison.previous.margin)}</span>
                  <span>Current: {formatCurrency(revisionMetrics.marginComparison.current.margin)}</span>
                </div>
              </div>
            </CardContent>
          </Card>
              ...
              <div className="flex flex-col">
                ...
                <div className="flex items-center mt-1 gap-1">
                  <Badge variant="outline" className="bg-green-50 text-green-700 hover:bg-green-100">
                    +{currentItems.filter(i => !compareItems.some(ci => ci.id === i.id || ci.description === i.description)).length} new
                  </Badge>
                  <Badge variant="outline" className="bg-red-50 text-red-700 hover:bg-red-100">
                    -{compareItems.filter(i => !currentItems.some(ci => ci.id === i.id || ci.description === i.description)).length} removed
                  </Badge>
                </div>
                <div className="mt-3 pt-3 border-t text-sm flex justify-between">
                  ...
                  >
                    ...
                    <RechartsTooltip
                      formatter={(value: number, name: string) => {
                        return [formatCurrency(value), name === 'difference' ? 'Difference' : name === 'currentValue' ? 'Current' : 'Previous'];
                      }}
                    />
                    <Legend />
                  ...
                  <tbody>
                    {revisionMetrics.categorySummary.map((category: any, index: number) => (
                      <tr key={index} className="border-b">
                        ...
                        <td className={`text-right py-2 ${category.difference >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                          {category.difference > 0 ? '+' : ''}
                          {formatCurrency(category.difference)}
                        </td>
                        <td className={`text-right py-2 ${category.percentChange >= 0 ? 'text-green-600' : 'text-red-600'}`}>
