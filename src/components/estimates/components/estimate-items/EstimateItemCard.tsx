...
const EstimateItemCard = memo(
  ({
    ...
    const { itemValues, calculatedValues } = useItemValues(index, form);
    ...
    // Explicit toggle handler to handle click on chevron
    const toggleItemOpen = useCallback(
      (e: React.MouseEvent) => {
        e.preventDefault();
        e.stopPropagation();
        console.log('Toggling item from', isOpen, 'to', !isOpen);
        setIsOpen(prev => !prev);
      },
      [isOpen]
    );
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <Badge
                  variant="outline"
                  className="flex items-center gap-1 bg-blue-50 cursor-pointer"
                >
    ...
    // Prevent entire content re-renders when closed
    const collapsibleContent = useMemo(
      ...
      [
        ...
        subcontractorSelector,
        documentAttachment,
        calculatedValues.itemPrice,
        calculatedValues.grossMargin,
        calculatedValues.grossMarginPercentage,
      ]
    );

    return (
      <div className="border rounded-md overflow-hidden transition-all">
        <div className="p-3 bg-white flex items-center gap-2" onClick={e => e.stopPropagation()}>
          <Button
            variant="ghost"
            size="sm"
          ...
          <div className="flex items-center gap-4">
            ...
            {showRemoveButton && <RemoveItemButton onRemove={onRemove} showButton={true} />}
          </div>
        </div>
