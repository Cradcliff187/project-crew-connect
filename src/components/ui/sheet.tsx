...
>(({ side = 'right', className, children, accessibleTitle = 'Sheet', ...props }, ref) => {
  ...
  // Similar approach to the Dialog component - check for a title and add one if not present
  React.useEffect(() => {
    if (!sheetContentRef.current) return;

    // Check if there's already a title element
    const hasTitleElement = !!sheetContentRef.current.querySelector('[role="dialog"] h2');
    ...
    if (!hasTitleElement) {
      ...
      sheetContentRef.current.prepend(hiddenTitle);
    }
  }, [accessibleTitle]);
