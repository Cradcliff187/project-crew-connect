...
const DueStatusBadge: React.FC<DueStatusBadgeProps> = ({ daysUntilDue, className }) => {
  ...
  if (daysUntilDue === 0) {
    ...
    return (
      <span
        className={cn(
          'inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-amber-100 text-amber-800',
          className
        )}
      >
        <CalendarClock className="h-3.5 w-3.5 mr-1.5" />
  ...
  return (
    <span
      className={cn(
        'inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800',
        className
      )}
    >
      <CalendarClock className="h-3.5 w-3.5 mr-1.5" />
