...
export function useMaterials(workOrderId: string) {
  ...
  const { materials, loading, error, fetchMaterials } = useMaterialsFetch(workOrderId);

  // Material CRUD operations
  const { submitting, handleAddMaterial, handleDelete } = useMaterialOperations(
    workOrderId,
    fetchMaterials
  );

  // Receipt operations
  const { handleReceiptUploaded } = useReceiptOperations(fetchMaterials);
