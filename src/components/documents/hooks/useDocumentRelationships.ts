...
export const useDocumentRelationships = (documentId?: string) => {
  ...
  const fetchRelationships = async () => {
    ...
    try {
      ...
      const docsWithRelationships = documents.map(doc => {
        ...
        const asSource = sourceRelationships.find(
          rel => rel.target_document_id === doc.document_id
        );
        const asTarget = targetRelationships.find(
          rel => rel.source_document_id === doc.document_id
        );
  ...
  const createRelationship = async (targetDocumentId: string, relationshipType: string) => {
    ...
    try {
      ...
      const { error } = await supabase.from('document_relationships').insert(relationshipData);
